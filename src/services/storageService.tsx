...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData?.error || response.statusText || "Error en la petición");
  }
...
export async function deleteTour(id: number): Promise<void> {
  return authRequest<void>(`/tours/${id}`, { method: "DELETE" });
}
...
export async function deleteStudyTrip(id: number): Promise<void> {
  return authRequest<void>(`/study-trips/${id}`, { method: "DELETE" });
}
